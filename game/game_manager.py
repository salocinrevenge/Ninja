...
class Game_Manager():
    ...
    def render(self):
        ...
        # Renderização Dinâmica em Formato Quadrado (Raio R)
        ...
        for dx in range(-r, r + 1):
            for dz in range(-r, r + 1):
                ...
                if neighbor_coord not in self.loaded_chunks:
                    self.loaded_chunks[neighbor_coord] = Game_Chunk(self, neighbor_x, neighbor_z, max_height=self.max_height)

    def update(self, dt):
        self.input()
        
        # Atualização Dinâmica de Chunks em Formato Quadrado (Raio R)
        r = self.render_distance_chunks
        for center in self.center_chunks:
            for dx in range(-r, r + 1):
                for dz in range(-r, r + 1):
